...
class PLC:
    ...
    # I2C,SPI & UART
    # - I2C
    SDA = board.GP0  # data out
    SCL = board.GP1  # data in
    ...
    def setPWM(self, freq=1000):
        ...
        try:
            self.QW0 = pwmio.PWMOut(
                board.GP5, duty_cycle=0, frequency=freq, variable_frequency=True
            )
            self.QW1 = pwmio.PWMOut(
                board.GP4, duty_cycle=0, frequency=freq, variable_frequency=True
            )
            print(
                f"Initialising PWM to {freq}Hz on board.GP4 & board.GP5"
            )

        except:
            print(
                "Error setting the PWM to {freq}Hz on board.GP4 & board.GP5"
            )
            self.AOUT0 = self.PWM0 = self.QW0 = None
            self.AOUT1 = self.PWM1 = self.QW1 = None
            return
    ...
    def setUART(self, baud=115200):
        """Initialiase the UART on GP0,GP1 @ 115200 baud"""
        print(
